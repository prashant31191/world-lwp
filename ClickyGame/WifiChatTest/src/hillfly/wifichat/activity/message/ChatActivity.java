...
public class ChatActivity extends BaseMessageActivity implements OnItemClickListener {
    ...
    public void finish() {
        ...
        if (null != mDBOperate) {// å…³é—­æ•°æ�®åº“è¿žæŽ¥
            mDBOperate.close();
            mDBOperate = null;
        }
        mRecordThread = null;
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (mAudioRecorder != null)
                    mAudioRecorder = null;
                mLayoutScroll.requestDisallowInterceptTouchEvent(false); // æ�¢å¤�ç›‘æµ‹æ»šåŠ¨äº‹ä»¶
                break;
    ...
    public boolean onLongClick(View v) {
        mLayoutScroll.requestDisallowInterceptTouchEvent(true); // å±�è”½æ»šåŠ¨äº‹ä»¶
        startRecord();
        return false;
    }
    ...
    public void processMessage(android.os.Message pMsg) {
        switch (pMsg.what) {
            ...
            case IPMSGConst.IPMSG_CONFIRM_IMAGE_DATA: { // å›¾ç‰‡å¼€å§‹å�‘é€�
                LogUtils.d(TAG, "æŽ¥æ”¶æ–¹ç¡®è®¤å›¾ç‰‡è¯·æ±‚,å�‘é€�çš„æ–‡ä»¶ä¸º" + mCameraImagePath);
            ...
            case IPMSGConst.IPMSG_CONFIRM_VOICE_DATA: { // è¯­éŸ³å¼€å§‹å�‘é€�
                LogUtils.d(TAG, "æŽ¥æ”¶æ–¹ç¡®è®¤è¯­éŸ³è¯·æ±‚,å�‘é€�çš„æ–‡ä»¶ä¸º" + mVoicePath);
                tcpClient = TcpClient.getInstance(ChatActivity.this);
                tcpClient.startSend();
            ...
            case IPMSGConst.IPMSG_CONFIRM_FILE_DATA: { // æ–‡ä»¶å¼€å§‹å�‘é€�
                LogUtils.d(TAG, "æŽ¥æ”¶æ–¹ç¡®è®¤æ–‡ä»¶è¯·æ±‚,å�‘é€�çš„æ–‡ä»¶ä¸º" + sendFilePath);
                tcpClient = TcpClient.getInstance(ChatActivity.this);
                tcpClient.startSend();
    ...
    public Handler handler = new Handler() {
        ...
        public void handleMessage(android.os.Message msg) {
            switch (msg.what) {
                case 0: // æ›´æ–°å½•éŸ³éŸ³é‡�
                    setDialogImage();
                    break;

                case 1:
                    refreshAdapter(); // æ›´æ–°æ¶ˆæ�¯æ˜¾ç¤º
                    break;
